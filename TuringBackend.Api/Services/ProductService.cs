...
namespace TuringBackend.Api.Services
{
    public class ProductService : IProductService
    {
        ...
        {
            var products = from pg in _dbContext.ProductCategory
                join p in _dbContext.Product on pg.ProductId equals p.ProductId
                where pg.CategoryId == category_id
                select p;
            return await products.ToPaginatedListAsync(page,limit,null);
        }

        public async Task<PaginatedList<Product>> GetProductsByDepartmentAsync(int department_id, int page, int limit,
            int description_length)
        {
            var products = from pc in _dbContext.ProductCategory
                join c in _dbContext.Category on pc.CategoryId equals c.CategoryId
                join p in _dbContext.Product on pc.ProductId equals p.ProductId
